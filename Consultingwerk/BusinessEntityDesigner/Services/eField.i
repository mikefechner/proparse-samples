...
DEFINE {&ACCESS} TEMP-TABLE {&PREFIX}eField{&SUFFIX} NO-UNDO {&REFERENCE-ONLY} &IF DEFINED (NO-BEFORE) EQ 0 &THEN BEFORE-TABLE eFieldBefore{&SUFFIX} &ENDIF
    ...
    FIELD FieldSerializeHidden AS LOGICAL FORMAT "yes/no":U INITIAL "FALSE":U
    ...
    FIELD FieldExtent AS INTEGER FORMAT ">>9":U INITIAL "0":U
    FIELD FieldGetAccess AS CHARACTER FORMAT "X(8)":U INITIAL "PUBLIC":U LABEL "&Get Access":T
    FIELD FieldSetAccess AS CHARACTER FORMAT "X(8)":U INITIAL "PUBLIC":U LABEL "&Set Access":T
    FIELD FieldCalcExpression AS CHARACTER FORMAT "X(800)":U LABEL "Calc Expression":T
    FIELD FieldCaseSensitive AS LOGICAL FORMAT "yes/no":U INITIAL "FALSE":U LABEL "Case Sensitive":T
