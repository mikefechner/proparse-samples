...
javafile = NEW java.io.File ("customer-tt.p").
...
PROCEDURE ProcessAst:
    ...
    /* Output details about Record Name Nodes */
    IF TYPE-OF (poNode, RecordNameNode) THEN DO:

        IF VALID-OBJECT (CAST (poNode, RecordNameNode):getTableBuffer()) THEN
            PUT UNFORMATTED FILL (" ", piNesting * 2 + 2) "getTableBuffer():fullName():            " CAST (poNode, RecordNameNode):getTableBuffer():fullName()  SKIP
                            FILL (" ", piNesting * 2 + 2) "getTableBuffer():getTable():getName():  " CAST (poNode, RecordNameNode):getTableBuffer():getTable():getName() SKIP .

        IF VALID-OBJECT (CAST (poNode, RecordNameNode):getBufferScope()) THEN
            PUT UNFORMATTED FILL (" ", piNesting * 2 + 2) "getBufferScope():getSymbol():getName(): " CAST (poNode, RecordNameNode):getBufferScope():getSymbol():getName() SKIP
                            FILL (" ", piNesting * 2 + 2) "getBufferScope():isStrong():            " CAST (poNode, RecordNameNode):getBufferScope():isStrong() " "
                                                                                    "isWeak(): " CAST (poNode, RecordNameNode):getBufferScope():isWeak() " " SKIP
                            FILL (" ", piNesting * 2 + 2) "getBufferScope():getBlock():getNode():  " CAST (poNode, RecordNameNode):getBufferScope():getBlock():getNode():ToString() SKIP .
    END.
